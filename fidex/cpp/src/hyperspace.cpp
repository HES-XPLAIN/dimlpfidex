...
Hyperspace::Hyperspace(char *hyperLocusFile) {
  ...
  if (fileLocus.fail()) {
    throw std::runtime_error("Error : file " + std::string(hyperLocusFile) +
                             " not found");
  }
...
vector<vector<double>> Hyperspace::getHyperLocus() { return hyperLocus; }

Hyperbox *Hyperspace::getHyperbox() { return hyperbox; }

void Hyperspace::ruleExtraction(vector<double> *mainSampleData,
                                const int mainSamplePred, double ruleAccuracy,
                                double ruleConfidence, vector<string> &lines) {
  ...
  for (int k = 0; k < hyperbox->getDiscriminativeHyperplans().size(); k++) {
    ...
    attribut = hyperbox->getDiscriminativeHyperplans()[k].first %
               (*mainSampleData).size();
    hypValue = hyperLocus[hyperbox->getDiscriminativeHyperplans()[k].first]
                         [hyperbox->getDiscriminativeHyperplans()[k].second];
    ...
    line += "X" + std::to_string(attribut) + inequality +
            std::to_string(hypValue) + " ";
  }
  line += "-> class " + std::to_string(mainSamplePred);
  line += " Covering size : " +
          std::to_string(hyperbox->getCoveredSamples().size());
...
double Hyperspace::computeRuleAccuracy(
    vector<int> *trainPreds, vector<int> *trainTrueClass,
    bool mainSampleCorrect) { // Percentage of correct model prediction on
...
double Hyperspace::computeRuleConfidence(
    vector<vector<double>> *trainOutputValuesPredictions,
    const int mainSamplePred,
    double mainSamplePredValue) { // Mean output value of prediction of class
  ...
  for (int i = 0; i < hyperbox->getCoveredSamples().size(); i++) {
    idSample = hyperbox->getCoveredSamples()[i];
    total += (*trainOutputValuesPredictions)
        [idSample][mainSamplePred]; // value of output prediction of class
                                    // chosen by the rule (mainSamplePred)
  }
