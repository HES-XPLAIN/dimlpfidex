...
void showParams() {
  ...
  std::cout << "fidexGloRules -T <train dataset file> -P <train prediction "
               "file> -C <train true class file> ";
  std::cout << "-H <hyperLocus file> ";
  std::cout << "-O <Rules output file> ";
  std::cout << "-M <Heuristic 1: optimal fidexGlo, 2: fast fidexGlo 3: very "
               "fast fidexGlo> ";
  std::cout << "<Options>\n\n";
...
int main(int nbParam, char **param) {
  try {
      ...
      if (*param[p] == '-') {
        ...
        case 'M':
          if (CheckPositiveInt(param[p]) && atoi(param[p]) >= 1 &&
            ...
            throw std::runtime_error(
                "Error : invalide type for parameter " + string(param[p - 1]) +
                ", heuristic must be an integer between 1 and 3(1: optimal "
                "fidexGlo, 2: fast fidexGlo 3: very fast fidexGlo)");
          }
          break;
    ...
    if (!rulesFileInit) {
      throw std::runtime_error(
          "The output rules file has to be given with option -O");
    }
    if (!heuristicInit) {
      throw std::runtime_error(
          "The heuristic(1: optimal fidexGlo, 2: fast fidexGlo 3: very fast "
          "fidexGlo) has to be given with option -M");
    }
    ...
    if ((*trainPreds).size() != nbDatas ||
        (*trainTrueClass).size() != nbDatas) {
      throw std::runtime_error(
          "All the train files need to have the same amount of datas");
    }
    ...
    std::iota(std::begin(notCoveredSamples), std::end(notCoveredSamples),
              0); // Vector from 0 to nbDatas-1

    vector<tuple<vector<tuple<int, bool, double>>, vector<int>, int, double,
                 double>>
        chosenRules; // antecedents, cover vector, class, rule accuracy, rule
                     // confidence
    ...
    if (heuristic == 1) {
      ...
      tuple<vector<tuple<int, bool, double>>, vector<int>, int, double, double>
          rule; // Ex: ([X2<3.5 X3>=4], covering, class)
      ...
      for (int idSample = 0; idSample < nbDatas; idSample++) {
        ...
        while (!ruleCreated) {
          ruleCreated = exp.fidex(
              rule, trainData, trainPreds, trainOutputValuesPredictions,
              trainTrueClass, &(*trainData)[idSample], (*trainPreds)[idSample],
              (*trainTrueClass)[idSample], &hyperspace, nbIn, nbAttributs,
              nbHyp, itMax, currentMinNbCov, dropoutDim, dropoutDimParam,
              dropoutHyp, dropoutHypParam);
          if (currentMinNbCov >= 2) {
            currentMinNbCov -= 1; // If we didnt found a rule with desired
                                  // covering, we check with a lower covering
          }
        }
      ...
      std::cout << "Number of sample with lower covering than " << minNbCover
                << " : " << nbProblems << endl;
      ...
      while (notCoveredSamples.size() != 0) {
        ...
        for (int r = 0; r < rules.size(); r++) {
          ...
          ite = std::set_difference(
              newNotCoveredSamples.begin(), newNotCoveredSamples.end(),
              get<1>(rules[r]).begin(), get<1>(rules[r]).end(),
        ...
        notCoveredSamples =
            bestNewNotCoveredSamples; // Delete new covered samples

        nbRules += 1;
        // std::cout << "Rule number " << nbRules << " ";
        // std::cout << "Size of covering : " << std::get<1>(currentRule).size()
        // << " ";
        chosenRules.push_back(
            currentRule); // add best rule with maximum covering
    ...
    if (heuristic == 2) {
      ...
      std::sort(dataIds.begin(), dataIds.end(),
                [&rules](int ruleBest, int ruleWorst) {
                  return std::get<1>(rules[ruleWorst]).size() <
                         std::get<1>(rules[ruleBest]).size();
                });
      ...
      while (notCoveredSamples.size() != 0) {
        ...
        ite = std::set_difference(
            notCoveredSamples.begin(), notCoveredSamples.end(),
            get<1>(currentRule).begin(), get<1>(currentRule).end(),
        ...
        if (ancienNotCoveringSize > notCoveredSamples.size()) {
          nbRules += 1;
          // std::cout << "Rule number " << nbRules << " ";
          // std::cout << "Size of covering : " <<
          // std::get<1>(currentRule).size() << " ";
          chosenRules.push_back(
              currentRule); // add best rule with maximum covering
          // cout << "nbRules : " << chosenRules.size() << " ";
          // std::cout << "New size : " << notCoveredSamples.size() << " ";
    ...
    if (heuristic == 3) {
      ...
      std::shuffle(std::begin(notCoveredSamples), std::end(notCoveredSamples),
      ...
      while (notCoveredSamples.size() != 0) {
        ...
        for (int j=0; j<get<0>(rule).size(); j++){
          cout  << std::get<0>(std::get<0>(rule)[j]) << " " <<
        std::get<1>(std::get<0>(rule)[j]) << " " <<
        std::get<2>(std::get<0>(rule)[j]) << endl;
        }
        ...
        */
        ...
        notCoveredSamples.erase(
            std::remove_if(notCoveredSamples.begin(), notCoveredSamples.end(),
                           [&rule](int x) {
                             return std::find(get<1>(rule).begin(),
                                              get<1>(rule).end(),
                                              x) != get<1>(rule).end();
                             // find index of coveredSamples which is x (x is
                             // element of notCoveredSamples), std::find returns
                             // last if x not found
                             // -> Removes x if it appears on coveredSamples
                             // (found before the end of coveredSamples)
                           }),
    ...
    std::iota(std::begin(RulesIds), std::end(RulesIds),
              0); // Vector from 0 to nbRules-1
    std::sort(RulesIds.begin(), RulesIds.end(),
              [&chosenRules](int ruleBest, int ruleWorst) {
                return std::get<1>(chosenRules[ruleWorst]).size() <
                       std::get<1>(chosenRules[ruleBest]).size();
              });
    ...
    for (int c = 0; c < nbRules; c++) { // each rule
      ...
      line = "";
      line += std::to_string(get<0>(chosenRules[r]).size()) +
              " : "; // If we need to specify the antecedant number
      for (int a = 0; a < get<0>(chosenRules[r]).size();
           a++) {                                     // each antecedant
        ...
        line += "X" + std::to_string(get<0>(get<0>(chosenRules[r])[a])) +
                inequality + std::to_string(get<2>(get<0>(chosenRules[r])[a])) +
                " "; // Write antecedant
      }
      line += "-> class " +
              std::to_string(std::get<2>(chosenRules[r])); // class of the rule
      line +=
          " Covering size : " +
          std::to_string(std::get<1>(chosenRules[r]).size()); // Covering size
      line += " Fidelity : 1";                                // Rule fidelity
      line += " Accuracy : " +
              std::to_string(std::get<3>(chosenRules[r])); // Rule accuracy
    ...
    if (file2.is_open()) {
      // We add to the file the number of rules, the mean covering per rule and
      // the mean number of antecedents per rule
      string startLine = "Number of rules : " + std::to_string(nbRules) +
                         ", mean sample covering number per rule : " +
                         std::to_string(meanCovSize) +
                         ", mean number of antecedents per rule : " +
                         std::to_string(meanNbAntecedents) + "\n";
      file2 << startLine;
      ...
      throw std::runtime_error("Error : Couldn't open rules extraction file " +
                               std::string(rulesFile) + ".");
    }

    std::cout << "Mean covering size per rule : " << meanCovSize << endl;
    std::cout << "Mean number of antecedents per rule : " << meanNbAntecedents
              << endl;
...
// Exemple : fidexGloRules -T datafiles/datanorm -P datafiles/dimlp.out -C
// datafiles/dataclass2 -H datafiles/hyperLocus -O datafiles/globalRules.txt -M
// 3 -i 100 -v 2 -d 0.5 -h 0.5
