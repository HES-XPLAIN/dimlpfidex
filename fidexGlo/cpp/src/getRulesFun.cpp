...
void getRules(
  ...
  tuple<vector<tuple<int, bool, double>>, int, int, double, double>
      rule; // <[X0<0.606994 X15>=0.545037], 12(cov size), 0(class),
            // 1(fidelity), 0.92(accuracy)>

  fstream rulesData;
  rulesData.open(rulesFile, ios::in); // Read data file
  if (rulesData.fail()) {
    throw std::runtime_error("Error : file " + std::string(rulesFile) +
                             " not found");
  }
  ...
  while (!rulesData.eof()) {
    ...
    if (line.length() != 0) {
      ...
      while (myLine >> value) {
        if (value[0] == 'X') { // New attribut
          ...
          if (attr >= nbTestAttributs) {
            throw std::runtime_error("Error : There is not enough attributs "
      ...
      rule = make_tuple(antecedents, covSize, ruleClass, ruleFidelity,
                        ruleAccuracy);
...
void getActivatedRules(
  ...
  for (int r = 0; r < (*rules).size(); r++) { // For each rule
    ...
    for (int a = 0; a < get<0>((*rules)[r]).size();
         a++) { // For each antecedent
      attr = get<0>(get<0>((*rules)[r])[a]);
      ineq = get<1>(get<0>((*rules)[r])[a]);
      val = get<2>(get<0>((*rules)[r])[a]);
      if (ineq == 0 &&
          (*testValues)[attr] >= val) { // If the inequality is not verified
        notActivated = true;
      }
