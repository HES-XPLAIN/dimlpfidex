...
String? iterableAsStringValueTransformer(String? value, Metadata metadata) {
  if (value == null || value.isEmpty) return null;

  // removing  spaces
  value = value.replaceAll(RegExp(r',(\ )+'), ',');

  if (metadata.datatype == Datatype.listFilePath) {
    // ensure no single brackets are present and surround value with brackets
    value = "[${value.replaceAll(RegExp(r'\[|\]'), '')}]";
  } else if (metadata.datatype == Datatype.dictionary) {
    value = "{${value.replaceAll(RegExp(r'\{|\}'), '')}}";
  }
...
List<dynamic>? listValueTransformer(String? value, Metadata metadata) {
  if (value == null || value.isEmpty) return null;
