...
DataSet::DataSet(char *dataFile, char *predFile, char *trueClassFile) {
  ...
  if (fileDta.fail()) {
    throw std::runtime_error("Error : file " + std::string(dataFile) +
                             " not found");
  }
  ...
  if (trueClassFile != 0) {
    ...
    if (fileCl.fail()) {
      throw std::runtime_error("Error : file " + std::string(trueClassFile) +
                               " not found");
    }
  ...
  if (filePrd.fail()) {
    throw std::runtime_error("Error : file " + std::string(predFile) +
                             " not found");
  }
  ...
  while (!filePrd.eof()) {
    ...
    if (line.length() != 0) {
      ...
      predictions.push_back(std::max_element(values.begin(), values.end()) -
                            values.begin());
    }
  }
...
DataSet::DataSet(char *weightFile) {
  ...
  if (fileWts.fail()) {
    throw std::runtime_error("Error : file " + std::string(weightFile) +
                             " not found");
  }
...
vector<vector<double>> *DataSet::getDatas() {
  if (hasDatas) {
    ...
    throw std::runtime_error(
        "Error : data file not specified for this dataset");
  }
}
...
vector<int> *DataSet::getTrueClasses() {
  if (hasClasses) {
    ...
    throw std::runtime_error(
        "Error : dataClass file not specified for this dataset");
  }
}
...
vector<int> *DataSet::getPredictions() {
  if (hasDatas) {
    ...
    throw std::runtime_error(
        "Error : prediction file not specified for this dataset");
  }
}
...
vector<vector<double>> DataSet::getWeights() {
  if (hasWeights) {
    ...
    throw std::runtime_error(
        "Error : weight file not specified for this dataset");
  }
}
