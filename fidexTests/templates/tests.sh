...
# --train_class_file <train true class file, not mandatory if classes are specified in train data file>
# --weights_file <weights file. In case of bagging, put prefix of files, ex: dimlpBT, files need to be in the form dimlpBTi.wts, i=1,2,3,... and you need to specify the number of networks with --nb_dimlp_nets> [Not mandatory if a rules file is given with -f]
# -f <rules file to be converted to hyperlocus> [Not mandatory if a weights file is given]
