...
public:
  ...
  void TrainAll(DataSet &masterTrain, DataSet &masterClass, DataSet &test,
                DataSet &testTar, char genericWeightsFile[],
                char *accuracyFile);
  ...
  void ComputeAcc(DataSet &data, DataSet &target, float *accuracy, int tW,
                  char predFile[]);

  void Del();

  BagDimlp(float eta, float mu, float flat, float errParam, float accuracyParam,
           float deltaErrParam, int discrLevels, int showErrParam,
           int nbEpochsParam, int nbLayers, int nbNeurons[], int nbDimlpNets,
           char weightFile[]);

  BagDimlp(int discrLevels, int nbLayers, int *nbNeurons, int nbDimlpNets,
           char weightFile[]);
};
