...
{  int max, newVal;
   ...
   if (nbRules == 0) return -1;

   if (nbRules == 1) return 0;
...
{  int   r, count;
   ...
   for (r=0, count=0, GoToBeg(); r<NbRules; r++, GoToNext())
   {
       if (IsRuleEmpty(GetRule()) == 1) continue;
...
{  int    a, r, t, var, nbThresOneVar, nbAnt, nbCarried, nbCarriedMod;
   ...
   for (r=0, count=0, GoToBeg(); r<nbRules; r++, GoToNext())
   {
       ...
       {
           if (oneCopy.IsAntDeleted() == 1) continue;
