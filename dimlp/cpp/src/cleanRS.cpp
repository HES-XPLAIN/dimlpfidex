...
{
  return (float)CountCarPatByFlags(flag) / (float)CountFlaggedRules(flag);
}
...
{
  return (float)CountFlaggedAnt(flag) / (float)CountFlaggedRules(flag);
}
...
void CleanRuleStruct::ComputeAcc(int indClean, StringInt *carried,
                                 int *indWrong, DataSet &data,
                                 DataSet &dataClass, int *nbPat, int *correct,
                                 int *wrong, float *acc)
...

{
  ...
  ComputeAcc(
      indClean, carried, WrongTrain, Train, TrainClass,
      &(Clean[indClean]->NbCarriedTrain), &(Clean[indClean]->NbCorrectTrain),
      &(Clean[indClean]->NbWrongTrain), &(Clean[indClean]->AccuracyTrain));
  ...
  if (Test.GetNbEx() != 0) {
    ...
    ComputeAcc(
        indClean, carried, WrongTest, Test, TestClass,
        &(Clean[indClean]->NbCarriedTest), &(Clean[indClean]->NbCorrectTest),
        &(Clean[indClean]->NbWrongTest), &(Clean[indClean]->AccuracyTest));
  ...
  if (Valid.GetNbEx() != 0) {
    ...
    ComputeAcc(
        indClean, carried, WrongValid, Valid, ValidClass,
        &(Clean[indClean]->NbCarriedValid), &(Clean[indClean]->NbCorrectValid),
        &(Clean[indClean]->NbWrongValid), &(Clean[indClean]->AccuracyValid));
...

{
  ...
  for (r = 0, Pruned->GoToBeg(); r < nbPrunedRules; r++, Pruned->GoToNext()) {
    ...
    SortAnt(Clean[countEffectRules]->SevAnt, Clean[countEffectRules]->NbAnt,
            countEffectRules);
...
CleanRuleStruct::CleanRuleStruct(DataSet all, DataSet train, DataSet trainClass,
                                 DataSet valid, DataSet validClass,
                                 DataSet test, DataSet testClass,
                                 RuleProcessing *processed, BpNN *bpNn,
                                 float *out, int nbOut)
...
int CleanRuleStruct::IsExampleCarried(DataSet &data, int index, CleanRule *rule)
...

{
  ...
  for (r = 0; r < NbRules + 1; r++) {
    if (Clean[r]->NbCarriedTrain == 0)
      ...
      Clean[r]->AccuracyTrain =
          (float)Clean[r]->NbCorrectTrain / (float)Clean[r]->NbCarriedTrain;
  }
}
...

{
  ...
  for (r = 0; r < NbRules + 1; r++) {
    if (Clean[r]->NbCarriedTest == 0)
      ...
      Clean[r]->AccuracyTest =
          (float)Clean[r]->NbCorrectTest / (float)Clean[r]->NbCarriedTest;
  }
...

{
  ...
  for (r = 0; r < NbRules + 1; r++) {
    if (Clean[r]->NbCarriedValid == 0)
      ...
      Clean[r]->AccuracyValid =
          (float)Clean[r]->NbCorrectValid / (float)Clean[r]->NbCarriedValid;
  }
}
...

{
  ...
  for (i = 0, ptrOldAnt = Clean[indOld]->SevAnt; i < oldNbAnt;
       i++, ptrOldAnt++) {
...

{
  ...
  for (p = 0; p < nbEx; p++) {
    ...
    for (r = 0, c = 0, z = 0; r < nbRules; r++) {
      if (IsExampleCarried(Test, p, Clean[r]) == 1) {
        z = 1;
        // cout << "-- " << r+1 << " " << 1 + Clean[r]->Classification <<
        // ClassPatNetTest[p] + 1 << "\n";

        cout << p + 1 << ": "
             << "-- R" << r + 1 << " " << 1 + Clean[r]->Classification << " "
             << clNet + 1 << " " << ClassPatNetTest[p] + 1 << "\n";
    ...
    if (z == 0)
      cout << p + 1 << ": "
           << "-- Default"
           << " " << 1 + Clean[nbRules]->Classification << " " << clNet + 1
           << " " << ClassPatNetTest[p] + 1 << "\n";
    cout << "\n";
  ...
  cout << "--- Accuracy on testing set = "
       << (float)globalAcc * 100.0 / (float)nbEx << "\n";
...

{
  ...
  for (p = 0; p < nbEx; p++) {
    ...
    for (r = 0, c = 0, z = 0; r < nbRules; r++) {
      if (IsExampleCarried(Test, p, Clean[r]) == 1) {
        ...
        cout << p + 1 << ": R" << r + 1 << " -- "
             << 1 + Clean[r]->Classification << " " << clNet + 1 << " "
             << ClassPatNetTest[p] + 1;
        if (Clean[r]->Classification != clNet)
          cout << " ***";
        if ((Clean[r]->Classification == clNet) &&
            (clNet != ClassPatNetTest[p]))
          cout << " ---";
        cout << "\n";
        Clean[r]->NbCarriedTest += 1;
    ...
    if (z == 0) {
      cout << p + 1 << ": "
           << " -- Net Class: " << clNet + 1
           << " True Class: " << ClassPatNetTest[p] + 1;
      if (clNet != ClassPatNetTest[p])
        cout << " ---";
      cout << "\n";
    }
  ...
  ruleFile << "--- Accuracy of rules on testing set = "
           << (float)globalAcc / (float)nbEx << "\n";
  ruleFile << "--- Fidelity on testing set (" << fidelity << "/" << nbEx
           << ") = " << (float)fidelity / (float)nbEx << "\n";
  ruleFile << "--- Accuracy when rules and network agree (" << globalWithout
           << "/" << fidelity
           << ") = " << (float)globalWithout / (float)fidelity << "\n\n";
  //   cout << "--- Contradictions = " << (float) contr / (float) nbEx << "\n";
  ruleFile
      << "--- Number of default rule activations (network classification) = "
      << countDef << "\n\n";
  //   cout << "--- NoFid = " <<  noFid << "\n";
