...

{
  ...
  cout << "-F <Folder where to save weights, predictions and datas after "
...

{
  ...
  int K = 10; // Number of divisions of the dataset (10 = 8 for train, 1 for
              // test and 1 for validation)
  ...
  string eta, mu, flat, errThres, accThres, deltaErr, showErr, epochs, quant,
      nbIn, nbOut, arch, arch2, attrFile, weightFile;
  char *ptrParam;
  ...
  if (N <= 0) {
    cout << "The number of times we do cross-validation must be at least 1, "
            "change the option N.\n";
    return -1;
  }

  if (K <= 2) {
    cout << "The number of divisions of the dataset must be at least 3, change "
            "the option -I.\n";
    return -1;
  }
  ...
  if (fileDta.fail()) {
    throw std::runtime_error("Error : file " + std::string(learnFile) +
                             " not found");
  }
  ...
  if (K > nbSamples) {
    cout << "The number of divisions of the dataset must be less or equal to "
            "the number of train samples("
         << nbSamples << "), change the option -I.\n";
    return -1;
  }
  ...
  if (fileDtaTar.fail()) {
    throw std::runtime_error("Error : file " + std::string(learnTar) +
                             " not found");
  }
  ...
  for (int n = 0; n < N; n++) {
    // Create folder
    string folderName =
        std::string(folder) + "\\Execution" + std::to_string(n + 1);
    string folderNameLinux =
        std::string(folder) + "/Execution" + std::to_string(n + 1) + "/";
    ...
    for (int k = 0; k < K; k++) { // K-fold, we shift groups by 1.
      // Create folder
      string folderName = std::string(folder) + "\\Execution" +
                          std::to_string(n + 1) + "\\Fold" +
                          std::to_string(k + 1);
      string folderNameLinux = std::string(folder) + "/Execution" +
                               std::to_string(n + 1) + "/Fold" +
                               std::to_string(k + 1) + "/";
#ifdef __unix__
      mkdir(folderNameLinux.c_str(), 0777);
...
#ifdef __unix__
      command += "-p " + std::string(folder) + "/Execution" +
                 std::to_string(n + 1) + "/Fold" + std::to_string(k + 1) +
                 "/train.out "; // Output train pred file
      command += "-t " + std::string(folder) + "/Execution" +
                 std::to_string(n + 1) + "/Fold" + std::to_string(k + 1) +
                 "/test.out "; // Output test pred file
      command += "-w " + std::string(folder) + "/Execution" +
                 std::to_string(n + 1) + "/Fold" + std::to_string(k + 1) +
                 "/weights.wts "; // Output weight file
#elif defined(_WIN32)
      command += "-p " + std::string(folder) + "\\Execution" +
                 std::to_string(n + 1) + "\\Fold" + std::to_string(k + 1) +
                 "\\train.out "; // Output train pred file
      command += "-t " + std::string(folder) + "\\Execution" +
                 std::to_string(n + 1) + "\\Fold" + std::to_string(k + 1) +
                 "\\test.out "; // Output test pred file
      command += "-w " + std::string(folder) + "\\Execution" +
                 std::to_string(n + 1) + "\\Fold" + std::to_string(k + 1) +
                 "\\weights.wts "; // Output weight file
#endif
...
#ifdef __unix__
      if (!copyFile("tempTrain.txt", (std::string(folder) + "/Execution" +
                                      std::to_string(n + 1) + "/Fold" +
                                      std::to_string(k + 1) + "/train.txt")
                                         .c_str())) {
        cout << "File tempTrain.txt coundn't be copied.\n";
        return -1;
      }
      if (!copyFile("tempTest.txt", (std::string(folder) + "/Execution" +
                                     std::to_string(n + 1) + "/Fold" +
                                     std::to_string(k + 1) + "/test.txt")
                                        .c_str())) {
        cout << "File tempTest.txt coundn't be copied.\n";
        return -1;
      }
      if (!copyFile("tempTarTrain.txt",
                    (std::string(folder) + "/Execution" +
                     std::to_string(n + 1) + "/Fold" + std::to_string(k + 1) +
                     "/trainTarget.txt")
                        .c_str())) {
