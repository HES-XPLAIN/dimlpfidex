...

{
  ...
  cout << "-w <output weight file>\n"; // If we want to specify weight output
                                       // file, not to be weights.wts
...
int dimlpTrn(string command) {
  ...
  for (k = 1; k < nbParam; k++) {
    if (commandList[k][0] == '-') {
      ...
      case 'W':
        weightFile = &(commandList[k])[0];
        break;

      case 'w':
        outputWeightFile = &(commandList[k])[0];
        break;
      ...
      case 'v':
        predValidationFile = &(commandList[k])[0];
        break;
  ...
  if (weightFile == 0)
    net = new Dimlp(eta, mu, flat, errThres, accThres, deltaErr, quant, showErr,
                    epochs, nbLayers, vecNbNeurons, outputWeightFile);

  else
    net = new Dimlp(weightFile, eta, mu, flat, errThres, accThres, deltaErr,
  ...
  net->Train(Train, TrainClass, Test, TestClass, Valid, ValidClass,
             accuracyFile);

  SaveOutputs(Train, net, nbOut, nbWeightLayers,
              predTrainFile); // Get train predictions
  if (testFile != 0) {
    SaveOutputs(Test, net, nbOut, nbWeightLayers,
                predTestFile); // Get test predictions
  }
  if (validFile != 0) {
    SaveOutputs(Valid, net, nbOut, nbWeightLayers,
                predValidationFile); // Get test predictions
  }
  if (ruleExtr) {
    if (attrFile != 0) {
    ...
    RealHyp ryp1(All, net, quant, nbIn, vecNbNeurons[1] / nbIn, nbWeightLayers);
    ...
    if (rulesFile != 0) {
      ...
      ryp1.RuleExtraction(All, Train, TrainClass, Valid, ValidClass, Test,
                          TestClass, Attr, rulesFileost);

      if (ryp1.TreeAborted()) {
        ryp1.Del();

        RealHyp2 ryp2(All, net, quant, nbIn, vecNbNeurons[1] / nbIn,
                      nbWeightLayers);
...
{
    dimlpTrn("DimlpTrn -L ../dimlp/datafiles/datanormTrain -1
../dimlp/datafiles/dataclass2Train -T ../dimlp/datafiles/datanormTest -2
../dimlp/datafiles/dataclass2Test -w ../dimlp/datafiles/dimlpDatanorm.wts -I 16
-H2 5 -O 2 -p ../dimlp/datafiles/dimlpDatanormTrain.out -t
../dimlp/datafiles/dimlpDatanormTest.out -R -F
../dimlp/datafiles/dimlpDatanormTrn.rls -o
../dimlp/datafiles/dimlpDatanormTrnStats -r
../dimlp/datafiles/dimlpDatanormTrnResult.txt");
}*/
