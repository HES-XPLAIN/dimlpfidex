...
class BpNN {
  ...
  void AssignParam(float eta, float mu, float flat, float errParam,
                   float accuracyParam, float deltaErrParam, int showErrParam,
                   int nbEpochsParam, int nbLayers, char saveFile[]);
  ...
  float ComputeErrorSameAct(DataSet &data, DataSet &target, float *accuracy);
...
public:
  ...
  void TrainPhase(DataSet &train, DataSet &trainTar, DataSet &test,
                  DataSet &testTar, DataSet &valid, DataSet &validTar,
                  char *accuracyFile, bool fromBT = false);
  ...
  BpNN(float eta, float mu, float flat, float errParam, float accuracyParam,
       float deltaErrParam, int showErrParam, int nbEpochsParam, int nbLayers,
       int nbNeurons[], char saveFile[], char printNetType[]);

  BpNN(char readFile[], int nbLayers, int nbNeurons[], char printNetType[]);

  BpNN(char readFile[], float eta, float mu, float flat, float errParam,
       float accuracyParam, float deltaErrParam, int showErrParam,
