...
public:
  ...
  void RuleExtraction(DataSet &data, DataSet &train, DataSet &trainClass,
                      DataSet &valid, DataSet &validClass, DataSet &test,
                      DataSet &testClass, AttrName &attr, ostream &ruleFile);

  //----------------------------------------------------------------

  void Del();

  RealHyp(DataSet &data, BpNN *nn, int nbBins, int nbIn, int multiple,
          int nbWeightLayers);

  RealHyp(DataSet &data, BpNN *nn, int nbBins, int nbIn, int multiple,
          int nbWeightLayers, int dummy);

  RealHyp(VirtualHyp *globalVirt, int nbNets, float *out, int nbOut,
