...

{
  ...
  for (l = 1; l < NbWeightLayers; l++)
    VecLayer[l] = new Layer(Eta, Mu, Flat, NbNeurons[l], NbNeurons[l + 1],
                            NbNeurons[l] * NbNeurons[l + 1], NbNeurons[l] + 1);
...
void BpNN::AssignParam(float eta, float mu, float flat, float errParam,
                       float accuracyParam, float deltaErrParam,
...
float BpNN::ComputeErrorSameAct(DataSet &data, DataSet &target, float *accuracy)
...

{
  ...
  VecLayer[0] = new Layer(Eta, Mu, Flat, NbNeurons[0], NbNeurons[1], nbWeights,
                          nbWeightsForInit);
...

{
  ...
  VecLayer[0] = new LayerDimlp(Eta, Mu, Flat, NbNeurons[0], NbNeurons[1],
                               nbWeights, nbWeightsForInit, discrLevels);
...

{
  ...
  VecLayer[0] = new LayerDimlp4(Eta, Mu, Flat, NbNeurons[0], NbNeurons[1],
                                nbWeights, nbWeightsForInit, discrLevels);
...
{
  VecLayer[1] = new LayerDimlp3(Eta, Mu, Flat, NbNeurons[1], NbNeurons[2],
                                NbNeurons[2], 2, discrLevels);

  VecLayer[2]->SetDown(VecLayer[1]->GetUp());
  VecLayer[2]->SetDeltaDown(VecLayer[1]->GetDeltaUp());

  VecLayer[0] = new LayerFdimlp(Eta, Mu, Flat, NbNeurons[0], NbNeurons[1],
                                NbNeurons[1], 2);
...
{
  VecLayer[1] = new LayerDimlp2(Eta, Mu, Flat, NbNeurons[1], NbNeurons[2],
                                NbNeurons[2], 2, discrLevels);
...

{
  VecLayer[1] = new LayerSD(NbNeurons[1], NbNeurons[2],
                            NbNeurons[1] * NbNeurons[2], NbNeurons[1] + 1);

  VecLayer[2]->SetDown(VecLayer[1]->GetUp());
  VecLayer[2]->SetDeltaDown(VecLayer[1]->GetDeltaUp());

  VecLayer[0] = new LayerDimlp(Eta, Mu, Flat, NbNeurons[0], NbNeurons[1],
                               NbNeurons[1], 2, discrLevels);
...
float BpNN::ComputeError(DataSet &data, DataSet &target, float *accuracy)
...
void BpNN::TrainPhase(DataSet &train, DataSet &trainTar, DataSet &test,
                      DataSet &testTar, DataSet &valid, DataSet &validTar,
                      char *accuracyFile, bool fromBT)

{
  float oldErr = 0;
  float acc, err, specAcc, specErr, prevValidErr, validErr, accValid, testErr,
      accTest;
  int e;
...
BpNN::BpNN(float eta, float mu, float flat, float errParam, float accuracyParam,
           float deltaErrParam, int showErrParam, int nbEpochsParam,
           int nbLayers, int nbNeurons[], char saveFile[], char printNetType[])
...
BpNN::BpNN(char readFile[80], int nbLayers, int nbNeurons[],
           char printNetType[])
...
BpNN::BpNN(char readFile[80], float eta, float mu, float flat, float errParam,
           float accuracyParam, float deltaErrParam, int showErrParam,
           int nbEpochsParam, int nbLayers, int nbNeurons[], char saveFile[],
           char printNetType[])
