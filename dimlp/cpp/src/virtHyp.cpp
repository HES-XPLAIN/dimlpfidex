    ...
    for (m = 0; m < Multiple; m++, ptrBias += NbIn, ptrWeights += NbIn) {
      ...
      for (k = 0; k < NbKnots; k++, ptrKnots++, ptrVar++) {
        if (*ptrWeights != 0.0)
          ...
          *ptrVar = (*ptrKnots - *ptrBias) * 1111111111.0 * 1111111111.0;
      }
    }
  }
}
...
VirtualHyp::VirtualHyp(int nbBins, int nbIn, int multiple, float *bias,
                       float *weights)
...
VirtualHyp::VirtualHyp(int nbBins, int nbIn, int multiple, int nbNets,
                       VirtualHyp **virt)
