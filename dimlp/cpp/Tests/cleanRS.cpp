...

{
  ...
  *wrong = 0;
  ...
  for (p = 0, carried->GoToBeg(); p < *nbPat; p++, carried->GoToNext()) {
    ...
    clNet = Bpnn->Max(Out, NbOut);
    cl = Bpnn->Max(dataClass.GetExample(indPat).data(), NbOut);
...

{
  ...
  const float *ptrPat = data.GetExample(index).data();
...

{
  ...
  for (p = 0; p < nbTrain; p++)
    ClassPatNetTrain[p] = Bpnn->Max(TrainClass.GetExample(p).data(), NbOut);

  if (nbTest > 0) {
    ClassPatNetTest = new int[nbTest];

    for (p = 0; p < nbTest; p++)
      ClassPatNetTest[p] = Bpnn->Max(TestClass.GetExample(p).data(), NbOut);
  }
  ...
  if (nbValid > 0) {
    ...
    for (p = 0; p < nbValid; p++)
      ClassPatNetValid[p] = Bpnn->Max(ValidClass.GetExample(p).data(), NbOut);
  }
}
