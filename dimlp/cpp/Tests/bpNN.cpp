...

{
  ...
  const vector<float> &example = data.GetExample(index); // To change if we find a way to have SetDown being a vector<float>
  vector<float> copyExample(example.begin(), example.end());
  VecLayer[0]->SetDown(copyExample.data());
...

{
  ...
  const vector<float> &example = data.GetExample(index);
  vector<float> copyExample(example.begin(), example.end());
  VecLayer[0]->SetDown(copyExample.data());
...

{
  ...
  for (p = 0, sum = 0.0, good = 0, bad = 0; p < nbPat; p++) {
    ...
    ptrTar.assign(target.GetExample(p).begin(), target.GetExample(p).end());
