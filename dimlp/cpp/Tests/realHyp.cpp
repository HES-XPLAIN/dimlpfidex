...

{
  ...
  for (p = 0; p < nbPat; p++) {
    ...
    const vector<float> &example = data.GetExample(p); // To change if we find a way to have SetDown being a vector<float>
    vector<float> copyExample(example.begin(), example.end());
    pat = copyExample.data();
...

{
  ...
  const vector<float> &example = data.GetExample(indPat); // To change if we find a way to have SetDown being a vector<float>
  vector<float> copyExample(example.begin(), example.end());
  In = copyExample.data();
