...
void Parameters::checkFilesIntegrity() {
  ...
  sanitizePath(EXPLANATION_FILE, true);
  sanitizePath(CONSOLE_FILE, true);
  sanitizePath(ATTRIBUTES_FILE, true);
  sanitizePath(WEIGHTS_FILE, true);
  sanitizePath(STATS_FILE, true);
...
void Parameters::sanitizePath(ParameterCode id, bool shouldFileExist) {
  ...
  if (!root.empty() && (root.back() == separator[0] || target.front() == separator[0])) {
    separator.clear();
  }

  string fullPath = shouldFileExist ? root + separator + target : root + separator;
